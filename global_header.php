...
$wanted_ad_id = intval(!empty($_REQUEST['wanted_ad_id'])?$_REQUEST['wanted_ad_id']:FALSE);
$meta_tags_details = meta_tags($_SERVER['PHP_SELF'], $parent_id, $auction_id, $wanted_ad_id);
...
while ($cats_header_details = $db->fetch_array($sql_select_cats_header))
{
	...
	//$parent_id = intval(!empty($_REQUEST['parent_id'])?$_REQUEST['parent_id']:FALSE); //defined above
	
	$categories_browse_box .= '<option value="' . $cats_header_details['category_id'] . '" ' . (($cats_header_details['category_id'] == $parent_id) ? 'selected' : '') . '>'.
