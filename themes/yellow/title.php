...
function header2($head_title) {
	global $setts;
	return header1($head_title." 2");
}

function header3($head_title) {
	global $setts;
	return header1($head_title." 3");
}

function header4($head_title) {
	global $setts;
	return header1($head_title." 4");
}

function header5($head_title) { //site status
	global $setts;
	return header1($head_title, "sideBoxHeader");
}

function header6($head_title) {
	global $setts;
	return header1($head_title." 6");
}

function header7($head_title) { // members
	global $setts;
	return header1($head_title, "sideBoxHeader");
}
function headercat($head_title) { // categories
	global $setts;
	return header1($head_title, "sideBoxHeader");
}
...
$parent_id = intval(!empty($_REQUEST['parent_id'])?$_REQUEST['parent_id']:FALSE);
$auction_id =  intval(!empty($_REQUEST['auction_id'])?$_REQUEST['parent_id']:FALSE);
$template->set('banner_header_content', $site_banner->select_banner($_SERVER['PHP_SELF'], $parent_id, $auction_id));
